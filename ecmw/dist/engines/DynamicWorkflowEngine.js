...
exports.DynamicWorkflowEngine = void 0;
...
class DynamicWorkflowEngine {
     ...
     */
    async synthesizeWorkflow(intent, context) {
     ...
     */
    async createWorkflowFromIntent(intent, context, workflowId) {
        const nodes = new Map();
        const edges = [];
        const entryPoints = [];
        const exitPoints = [];
     ...
     * Create nodes based on intent
     */
    async createIntentNodes(intent, context) {
        const nodes = [];
        // Primary intent node
     ...
     */
    createIntentNode(intent, analysis, context, priority) {
     ...
     */
    createParallelNode(intents, analysis, context) {
        const subNodes = intents.map((intent, index) => this.createIntentNode(intent, analysis, context, index + 2));
        return {
     ...
     */
    createContextNodes(context, intent) {
        const nodes = [];
     ...
     */
    createWorkflowEdges(entryNode, intentNodes, intent) {
        const edges = [];
            ...
            node.dependencies.forEach(depId => {
                ...
                if (dependencyNode) {
                    edges.push({
                        ...
                        to: node.id,
                        weight: 1,
                        dataFlow: dependencyNode.outputs.filter(output => node.inputs.includes(output))
                    });
                }
            });
        });
     ...
     */
    findLastNodes(nodes) {
        return nodes.filter(node => !nodes.some(otherNode => otherNode.dependencies.includes(node.id) && otherNode.id !== node.id));
    }
    /**
     * Calculate workflow metadata
     */
    calculateWorkflowMetadata(nodes, edges) {
     ...
     */
    determineComplexity(nodeCount, edgeCount) {
        if (nodeCount <= 3 && edgeCount <= 3)
            return 'simple';
        if (nodeCount <= 8 && edgeCount <= 12)
            return 'medium';
        return 'complex';
    }
    /**
     ...
     */
    async executeWorkflowRecursive(workflow, execution, context) {
     ...
     */
    findExecutableNodes(workflow, execution) {
        return Array.from(workflow.nodes.keys()).filter(nodeId => {
            ...
            return node.dependencies.every(depId => execution.completedNodes.includes(depId) ||
     ...
     */
    async executeServiceNode(node, context) {
     ...
     */
    async executeDecisionNode(node, context, execution) {
     ...
     */
    async executeParallelNode(node, context, execution) {
     ...
     */
    async executeSequentialNode(node, context, execution) {
     ...
     */
    async executeFallbackNodes(fallbackNodeIds, workflow, execution, context) {
     ...
     */
    async optimizeWorkflow(workflow, intent, context) {
     ...
     */
    generateCacheKey(intent, context) {
        const keyData = {
     ...
     * Get cached workflow
     */
    getCachedWorkflow(cacheKey) {
        return this.workflowCache.get(cacheKey);
    }
    /**
     * Cache workflow
     */
    cacheWorkflow(cacheKey, workflow) {
     ...
     */
    createGenericNode(intent, priority) {
        return {
...
exports.DynamicWorkflowEngine = DynamicWorkflowEngine;
exports.default = DynamicWorkflowEngine;
//# sourceMappingURL=DynamicWorkflowEngine.js.map