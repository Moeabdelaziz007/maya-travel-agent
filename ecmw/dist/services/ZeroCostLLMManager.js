...
exports.ZeroCostLLMManager = void 0;
const ECMWCore_1 = require("../core/ECMWCore");
class ZeroCostLLMManager {
    constructor(config) {
    ...
    calculateProviderScore(provider, request) {
        ...
        const capabilityMatches = request.requiredCapabilities.filter(cap => provider.strengths.includes(cap)).length;
     ...
     */
    async executeWithProvider(request, provider) {
        const startTime = Date.now();
        try {
     ...
     */
    async tryFallbackProviders(request) {
     ...
     */
    async callLLMProvider(request, provider) {
     ...
     */
    generateMockResponse(request, provider) {
        const responses = {
            intent_analysis: [
    ...
    assessResponseQuality(response, request) {
        ...
        const relevanceScore = promptKeywords.filter(word => responseKeywords.includes(word)).length / promptKeywords.length;
     ...
     * Update provider performance metrics
     */
    updateProviderMetrics(response) {
    ...
    async processRequestQueue() {
        ...
        // Remove from queue
        this.requestQueue = this.requestQueue.filter(req => !requestsToProcess.includes(req));
        // Process requests
...
exports.ZeroCostLLMManager = ZeroCostLLMManager;
exports.default = ZeroCostLLMManager;
//# sourceMappingURL=ZeroCostLLMManager.js.map