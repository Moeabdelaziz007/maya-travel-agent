...
import { ECMWCore, ECMWConfig, UserContext, WorkflowResult } from '../core/ECMWCore';

describe('ECMWCore', () => {
  let ecmw: ECMWCore;
  let config: ECMWConfig;
  ...
  describe('Initialization', () => {
    ...
    it('should initialize with minimal features when disabled', () => {
      const minimalConfig: ECMWConfig = {
      ...
      const minimalECMW = new ECMWCore(minimalConfig);
  ...
  describe('Request Processing', () => {
    it('should process a simple travel planning request', async () => {
      ...
      const result = await ecmw.processRequest(
        'test_user_1',
    ...
    it('should handle emotional context in requests', async () => {
      ...
      const result = await ecmw.processRequest(
        'test_user_2',
        'I am so excited about my upcoming adventure trip!',
        userContext
      );
  ...
  describe('User Context Management', () => {
    ...
    it('should update existing user context', async () => {
      ...
      const result = await ecmw.processRequest(
        ...
        { ...userContext, preferences: { ...userContext.preferences, city: 'Paris' } }
      );
...
describe('Integration Tests', () => {
  ...
  it('should handle complex multi-intent travel request', async () => {
    ...
    const result = await ecmw.processRequest(
      'complex_user',
      'I want to plan a luxury cultural trip to Morocco with my family, focusing on halal dining and historical sites',
      userContext
    );
  ...
  it('should learn from repeated interactions', async () => {
    ...
    const results = await Promise.all(
      requests.map(request =>
        ecmw.processRequest(userId, request, baseContext)
      )
    );
...
describe('Performance Tests', () => {
  ...
  it('should maintain performance under concurrent load', async () => {
    ...
    const allRequests = Array.from({ length: concurrentRequests }, (_, userIndex) =>
      Array.from({ length: requestsPerUser }, (_, requestIndex) =>
        ecmw.processRequest(
          `concurrent_load_user_${userIndex}`,
          `Concurrent load request ${requestIndex}`,
          {
...
describe('Error Recovery Tests', () => {
  ...
  it('should recover from partial failures', async () => {
    ...
    const result = await ecmw.processRequest(
      'recovery_user',
      'Complex travel request that might need fallback handling',
      userContext
    );
