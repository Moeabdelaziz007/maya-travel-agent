...
export const destinationService = {
  ...
  searchDestinations: (query: string) =>
    api.get(`/destinations/search?q=${query}`),
};
...
export const analyticsService = {
  track: (event: { type: string; userId?: string; payload?: any }) =>
...
export const budgetService = {
  // Get budget summary
  getBudgetSummary: (tripId?: string) =>
    api.get(`/budget/summary${tripId ? `?tripId=${tripId}` : ''}`),
  ...
  getExpenses: (tripId?: string) =>
    api.get(`/budget/expenses${tripId ? `?tripId=${tripId}` : ''}`),
};
