...
export class TestDataGenerator {
  static generateUser() {
    const firstNames = ['أحمد', 'محمد', 'فاطمة', 'عائشة', 'سارة', 'نور', 'لينا', 'جود']
    const lastNames = ['محمد', 'أحمد', 'علي', 'حسن', 'حسين', 'عباس', 'كريم', 'رحمان']
  ...
  static generateTripRequest() {
    ...
    const tripTypes = ['cultural', 'adventure', 'relaxation', 'business', 'family']
    ...
    return {
      ...
      preferences: {
        accommodation: ['hotel', 'resort', 'apartment'][Math.floor(Math.random() * 3)],
        activities: ['sightseeing', 'food', 'shopping', 'adventure'][Math.floor(Math.random() * 4)],
...
export class UiHelpers {
  static async fillTripPlannerForm(page: Page, tripData: any) {
    ...
    await page.fill('[data-testid="travelers-input"]', tripData.travelers.toString())
    await page.fill('[data-testid="budget-input"]', tripData.budget.toString())

    if (tripData.tripType) {
      await page.selectOption('[data-testid="trip-type-select"]', tripData.tripType)
    }
  ...
  static async navigateToTab(page: Page, tabName: string) {
    ...
    await page.waitForSelector(`[data-testid="${tabName.toLowerCase().replace(' ', '-')}-tab"]`)
  }
...
export class AssertionHelpers {
  ...
  static async assertChatResponse(page: Page, expectedContent?: string) {
    ...
    if (expectedContent) {
      await page.locator('[data-testid="ai-response"]').filter({ hasText: expectedContent }).isVisible()
    }
  }
  ...
  static async assertEmotionDetection(page: Page, expectedEmotion?: string) {
    ...
    if (expectedEmotion) {
      await page.locator(`[data-testid="emotion-indicator"][data-emotion="${expectedEmotion}"]`).isVisible()
    }
  }
  ...
  static async assertFriendshipProgression(page: Page, expectedLevel?: string) {
    ...
    if (expectedLevel) {
      await page.locator(`[data-testid="friendship-level"]:has-text("${expectedLevel}")`).isVisible()
    }
  }
  ...
  static async assertErrorMessage(page: Page, expectedError?: string) {
    ...
    if (expectedError) {
      await page.locator(`[data-testid="error-message"]:has-text("${expectedError}")`).isVisible()
    }
  }
}
...
export class AccessibilityHelpers {
  static async checkPageAccessibility(page: Page) {
    ...
    for (const input of inputs) {
      ...
      if (!hasLabel) {
        console.warn('Input missing proper label:', await input.getAttribute('name'))
      }
    }
  }
