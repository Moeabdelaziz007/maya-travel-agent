...
const BudgetTracker: React.FC<BudgetTrackerProps> = ({ trips }) => {
  ...
  const getExpensesByCategory = () => {
    ...
    return Object.entries(categories).map(([category, amount]) => ({
      ...
      amount,
      percentage:
        getTotalExpenses() > 0 ? (amount / getTotalExpenses()) * 100 : 0,
    }));
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Monitor your travel expenses and stay on budget
        </p>
      </div>
          ...
          <div>
            ...
            <p className={`text-sm ${budgetStatus.color}`}>
              {budgetStatus.status === 'good'
                ...
                : budgetStatus.status === 'warning'
                ? 'Consider reducing expenses to stay within budget.'
        ...
        >
          ...
          <div className="space-y-4">
            {expenses
              .filter(
                expense =>
                  selectedTrip === 'all' || expense.tripId === selectedTrip
              )
              .slice(0, 5)
              .map((expense, index) => (
                <motion.div
